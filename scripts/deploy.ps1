...
foreach ($item in @("app", "scripts", "tests", "config", "docs", "requirements.txt", "config.yaml", ".env.example", "secrets.example.txt", "secrets.example.json", "README.md", "SECURITY.md")) { scp -r -- "$item" $target }
