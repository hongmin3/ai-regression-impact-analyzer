...
7. 데이터를 주고받아야 하면 **HTTP API로만** 한다. 그때 지킬 것 세 가지:
   설정·자격증명이 없으면 연동이 꺼진 채 기존 동작을 유지할 것, 상대가 죽어도 그 단계만
   건너뛰고 계속 진행할 것, 자격증명은 `secrets.*`에 두고 로그에 남기지 말 것.
   현재 예: `app/core/manual_hub_client.py` (매뉴얼 개정 검증의 Cross-Manual 대조).
