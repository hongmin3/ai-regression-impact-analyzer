...
class RegressionAnalyzer:
    ...
    def run(self, change_path: Path, specification_path: Path, testcase_path: Path, analysis_id: str | None = None) -> AnalysisResult:
        ...
        try:
            ...
            result = AnalysisResult(analysis_id=analysis_id, created_at=datetime.now(timezone.utc), change_file=change_path.name, specification_file=specification_path.name, testcase_file=testcase_path.name, change=change, total_tc=len(cases), candidate_tc=len(candidates), decisions=decisions, token_usage=self.gemini.token_usage)
