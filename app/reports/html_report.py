...
def create_csv_export(result: AnalysisResult) -> Path:
    ...
    with output.open("w", newline="", encoding="utf-8-sig") as handle:
        ...
        writer.writerow(["TC ID", "Impact", "Confidence", "Evidence Level", "Revision Mark", "Reason", "Related Specification", "Verification Point", "Recommended", "Manual Review"])
        for item in result.decisions:
            writer.writerow([item.tc_id, item.impact.value, item.confidence, item.evidence_level.value, item.revision_mark.value, item.reason, " | ".join(item.relevant_specifications), " | ".join(item.verification_points), item.recommended, item.manual_review_required])
    return output
...
def create_xlsx_export(result: AnalysisResult) -> Path:
    ...
    headers = ["TC ID", "Impact", "Confidence", "Evidence Level", "Revision Mark", "Reason", "Related Specification", "Verification Point", "Recommended", "Manual Review"]
    sheet.append(headers)
    for cell in sheet[1]:
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill("solid", fgColor="176B87")
    for item in result.decisions:
        sheet.append([item.tc_id, item.impact.value, item.confidence, item.evidence_level.value, item.revision_mark.value, item.reason, " | ".join(item.relevant_specifications), " | ".join(item.verification_points), item.recommended, item.manual_review_required])
    sheet.freeze_panes = "A2"
    sheet.auto_filter.ref = sheet.dimensions
    widths = (18, 12, 12, 16, 14, 48, 35, 35, 14, 14)
